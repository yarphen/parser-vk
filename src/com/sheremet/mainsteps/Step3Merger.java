...
import com.sheremet.threads.MergerThread;
import com.sheremet.utils.JTextAreaOutputStream;

@SuppressWarnings("deprecation")
public class Step3Merger extends JFrame{
	...
	private MergerThread mergerThread;
			...
			public void actionPerformed(ActionEvent arg0) {
				if (started==false){
					if (JOptionPane.showOptionDialog(null, 
						...
						log.setText("");
						mergerThread = new MergerThread(input.getText().split("\n"),new ActionListener() {
							@Override
							public void actionPerformed(ActionEvent arg0) {
								mergerThread = null;
					...
					if (JOptionPane.showOptionDialog(null, 
							...
							JOptionPane.QUESTION_MESSAGE, 
							null, null, null)==JOptionPane.YES_OPTION){
						PrintStream pw = mergerThread.getPS();
						if (pw!=null)
							synchronized (pw) {
								pw.close();
							}
						Scanner sc = mergerThread.getScanner();
						if (sc!=null)
							synchronized (sc) {
								sc.close();
							}
						pw = mergerThread.getCurFilePS();
						if (pw!=null)
							synchronized (pw) {
								pw.close();
							}
